...
- `GET /api/metadata?mode=editions&q=…` — drills into per-provider edition listings (legie `/vydani`, databazeknih `/dalsi-vydani/<slug>` across language tabs) and returns `{ results, sources }` with one `BookSearchResult` per published edition (per-edition `isbn`, `coverUrl`, `publisher`, `year`, `language`; bibliographic fields shared from the master record). Dedup key is ISBN, falling back to `coverUrl::publisher` so different printings stay distinct.
...
- **[types.ts](server/services/plugins/types.ts)** — `BookSourcePlugin` interface with optional `searchByISBN`, `searchByTitle`, `searchByAuthor`, `searchBySeries`, `searchByText`, `findCovers`, `searchEditions`. Each plugin implements only what it supports. `PluginConfig = { order, disabled }` threads user prefs into the runner.
- **[registry.ts](server/services/plugins/registry.ts)** — `registerPlugin`, `pluginsFor(capability)`.
- **[runner.ts](server/services/plugins/runner.ts)** — `runByISBN / runByTitle / runByAuthor / runBySeries / runByText / runCoverSearch / runEditions`. Orchestrates parallel execution with per-plugin timeout. Merge strategy: for each field, take the value from the highest-priority candidate with a non-empty value. ISBN lookups run in two phases — phase 1 queries all ISBN-capable plugins; phase 2 uses the discovered title to run title-only enrichers (e.g. legie.info) whose results are then priority-sorted together. `runEditions` keeps each printing distinct (different ISBN or `coverUrl::publisher`) so a SK+CZ release of the same title doesn't collapse into one row.
...
- **[src/services/bookApi.ts](src/services/bookApi.ts)** — thin wrapper. `fetchByISBN / searchByTitle / searchByAuthor / searchBySeries / searchByText / searchEditions` each hit `/api/metadata?mode=...`. `searchEditions` powers the BookDetail "Find editions" picker.
