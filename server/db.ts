...
const DATA_DIR = join(process.env.HOME ?? __dirname, ".local", "share", "my-library");
...
export function getAllBooks(): Book[] {
  return (db.prepare("SELECT * FROM books ORDER BY addedAt DESC").all() as Record<string, unknown>[]).map(toBook);
}
...
export function searchBooks(query: string): Book[] {
  ...
  `).all(like, like, like, query) as Record<string, unknown>[]).map(toBook);
}
...
export function importBooks(books: Book[]): number {
  ...
  const insertMany = db.transaction((rows: Book[]) => {
    for (const b of rows) {
      const r = serialize(b);
      stmt.run(
        r.id, r.isbn ?? null, r.title, r.authors, r.genres,
        r.description ?? "", r.publisher ?? "", r.pageCount ?? null,
        r.series ?? "", r.seriesNumber ?? "", r.coverUrl ?? "",
        r.coverBase64 ?? "", r.averageRating ?? null, r.ratingsCount ?? null,
        r.isRead ?? 0, r.notes ?? "", r.source ?? "manual",
