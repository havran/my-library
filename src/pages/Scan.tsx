...
export default function Scan() {
  ...
  const [seriesWizard, setSeriesWizard] = useState<{
    seriesTitle: string;
    serieSlug: string;
    bookTitle: string;
  } | null>(null);
  ...
  const [isbnCapturing, setIsbnCapturing] = useState(false);
  const [isbnDebug, setIsbnDebug] = useState<{ raw: string; image: string } | null>(null);
  ...
  const handleAddBook = useCallback(
    async (data: BookSearchResult, source: Book["source"]) => {
      setIsAdding(true);
  ...
  const handleISBN = useCallback(
    async (isbn: string, source: Book["source"]) => {
  ...
  function measureSharpness(src: HTMLCanvasElement): number {
    const W = 200,
      H = Math.max(1, Math.round((200 * src.height) / src.width));
  ...
  const captureISBN = useCallback(async () => {
    const video = isbnVideoRef.current;
    if (!video || !isbnActiveRef.current || isbnCapturing) return;
    if (scannedRef.current || isLoadingRef.current) return;

    setIsbnCapturing(true);
    setIsbnFeedback("");

    const BURST = 6;
    const BURST_INTERVAL = 120;

    // Pick the sharpest frame from a short burst so a shaky tap still gets a good read.
    ...
    if (!best) {
      setIsbnCapturing(false);
      setIsbnFeedback("Couldn't grab frame — try again");
      return;
    }
    ...
    try {
      ...
      const res = await apiFetch("/api/isbn-ocr", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image: base64, debug: true }),
        signal: isbnAbortRef.current.signal,
      });
      const { isbn, partial, raw, debugImage } = (await res.json()) as {
        isbn: string | null;
        partial: string | null;
        raw?: string;
        debugImage?: string;
      ...
      console.debug("[isbn-ocr]", { isbn, partial, raw, sharpness: Math.round(bestScore) });
      if (debugImage) {
        setIsbnDebug({ raw: raw ?? "", image: `data:image/png;base64,${debugImage}` });
      }
      if (isbn && !scannedRef.current && !isLoadingRef.current) {
        ...
        setIsbnFeedback(partial ? `Partial: ${partial}` : "No ISBN detected — try again");
      }
    } catch (e) {
      if ((e as Error)?.name !== "AbortError") {
        setIsbnFeedback("OCR failed — try again");
      }
    } finally {
      setIsbnCapturing(false);
    }
  }, [handleISBN, isbnCapturing]);
  ...
  const handleCaptureAndOCR = async () => {
    ...
    if (results.length > 0) {
      ...
      showToast(
        "Couldn't read text from cover. Try better lighting or use text search below.",
        "error",
      );
      setShowTextSearch(true);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black flex flex-col z-50">
        ...
        <>
          {permissionDenied ? (
            ...
            <>
              ...
              {/* OCR feedback */}
              {isbnFeedback && (
                <div className="absolute bottom-36 inset-x-8 bg-black/70 backdrop-blur rounded-2xl px-4 py-3 text-center pointer-events-none">
              ...
              {/* Debug: preprocessed image + raw OCR text */}
              {isbnDebug && (
                <div className="absolute top-24 inset-x-4 bg-black/70 backdrop-blur rounded-xl p-2 text-[11px] text-white/70 flex gap-2 items-start max-h-40 overflow-hidden">
                  <img
                    src={isbnDebug.image}
                    alt="preprocessed"
                    className="h-16 w-auto object-contain border border-white/20 rounded"
                  />
                  <div className="flex-1 min-w-0">
                    <p className="uppercase tracking-wider text-white/40 mb-1">OCR raw</p>
                    <p className="font-mono break-all leading-tight">{isbnDebug.raw || "∅"}</p>
                  </div>
                </div>
              )}

              {/* Capture button — single-shot OCR */}
              <div className="absolute bottom-0 inset-x-0 flex flex-col items-center pb-10 gap-3 pointer-events-none">
                <button
                  onClick={captureISBN}
                  disabled={isbnCapturing || isLoading}
                  className="pointer-events-auto w-18 h-18 rounded-full border-4 border-white bg-white/20 hover:bg-white/30 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center shadow-xl"
                  style={{ width: 72, height: 72 }}
                  aria-label="Capture ISBN"
                >
                  {isbnCapturing ? (
                ...
                {isbnCapturing && (
                  <p className="text-white/80 text-sm animate-pulse">Reading ISBN…</p>
                )}
              </div>
          ...
          {permissionDenied ? (
            ...
            <>
              ...
              <div className="absolute top-20 inset-x-0 flex flex-col items-center gap-2 pointer-events-none">
                ...
                {videoRes && (
                  <p
                    className={`text-xs px-3 py-1 rounded-full ${videoRes.w < 720 ? "bg-red-500/70 text-white" : "bg-black/30 text-white/50"}`}
                  >
                    {videoRes.w}×{videoRes.h}
                    {videoRes.w < 720 ? " — low resolution, OCR may fail" : ""}
                  </p>
                )}
              </div>
              ...
              <div className="absolute bottom-0 inset-x-0">
                <div
                  className="absolute bottom-0 inset-x-0 bg-black/70 backdrop-blur rounded-t-3xl transition-all"
                  style={{
                    transform: showTextSearch ? "translateY(0)" : "translateY(calc(100% - 48px))",
                  }}
                >
                  <button
      ...
      {toast && (
        <div
          className={`absolute bottom-10 inset-x-4 flex items-center gap-3 px-3 py-2.5 rounded-2xl text-sm font-medium shadow-lg animate-fade-in ${
            toast.type === "error"
        ...
        >
          {toast.cover && (
            <img
              src={toast.cover}
              alt=""
              className="w-9 h-[54px] rounded-lg object-cover shrink-0"
            />
          )}
          <span className="flex-1 line-clamp-2 leading-snug">{toast.msg}</span>
          <button onClick={() => setToast(null)} aria-label="Dismiss" className="shrink-0">
